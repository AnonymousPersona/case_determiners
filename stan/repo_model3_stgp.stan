// BM-free model, written to be directly comparable to the fuller model:
// - keeps same `tau` prior as the fuller model
// - assigns all latent scale to the separable space-time GP (sigma_st = tau)
// - removes all phylogeny-related data, parameters and code
// - keeps identical priors for ell_space, ell_time, sigma_eps, and other components
...
data {
  ...
  // -------- NOTE: phylogeny inputs removed (no M / K_phy / log_w)
...
transformed data {
  // (No phylogeny choleskies to precompute)
}
...
parameters {
  ...
  // reparameterization for magnitudes: keep same `tau` as in the fuller model
  real<lower=0> tau;                 // total GP scale (all assigned to space-time here)
  // NOTE: rho removed (no phylogenetic share)
}
...
model {
  // -------- Baselines (match fuller model's priors)
  ...
  // total magnitude: keep identical prior to full model for comparability
  tau ~ normal(0, 0.25) T[0, ];

  // nugget: same prior as full model
  //sigma_eps ~ normal(0, 0.05) T[0, ];
  sigma_eps ~ normal(0, 0.05) T[0, ];

  // Length-scales: same priors as full model
